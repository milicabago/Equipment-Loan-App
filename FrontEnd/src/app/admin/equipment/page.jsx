...
const EquipmentPage = () => {
    ...
    const [filterValues, setFilterValues] = useState({ name: '', condition: '', quantity: [1, 100] });
    ...
    const [incorrectEquipment, setIncorrectEquipment] = useState('');
    ...
    useEffect(() => {
        const filtered = equipment.filter(item =>
            ...
            (filterValues.condition === '' || item.condition?.toString() === filterValues.condition) &&
    ...
    const resetFilters = () => {
        setFilterValues({ name: [], condition: '', quantity: [1, 100] });
    };
    ...
    const handleSave = async () => {
        try {
       /* if (invalid_quantity > quantity) {
            toast.error("Invalid quantity cannot be greater than total quantity.", { duration: 3000 });
            return; 
        }*/
          ...
          const { name, full_name, model, serial_number, quantity, condition, description, invalid_quantity  } = editedEquipmentData;
          const updatedCondition = quantity === 0 ? "non-functional" : condition;

          const dataToUpdate = { name, full_name, model, serial_number, quantity, condition, description: updatedCondition, invalid_quantity};
        ...
        <div className={styles.container}>
             <div>
                <div className={styles.title}>
                    <h1>Equipment</h1>
                    <div>
                    <MdPlaylistAdd className={styles.addIcon} onClick={() => router.push('/admin/addEquipment')} />
            ...
            >
                ...
                <div className={styles.modalContent}>
                    ...
                    <p>
                    <span className={styles.label}>Invalid Quantity: </span>
                    <span>
                    <input
                        type="number"
                        name="quantity"
                        value={editedEquipmentData.invalid_quantity}
                        onChange={(e) => handleEdit('invalid_quantity', e.target.value)}
                        className={styles.input}
                        min={1}
                        autoComplete='off'
                    />
